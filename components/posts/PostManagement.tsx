...
const PostManagement: React.FC<PostManagementProps> = ({ posts }) => {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4">
      <h1 className="text-2xl font-bold text-gray-800 mb-4 text-center">Posts</h1>
      <ul className="space-y-2 w-full max-w-4xl">
        {posts?.map(post => (
          <li key={post.id} className="p-4 bg-white rounded-lg shadow-md hover:shadow-lg transition-shadow duration-300 ease-in-out flex flex-col ">
